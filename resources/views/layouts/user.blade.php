...
<head>
    ...
    <style>
            /* CSS Pendukung untuk menyembunyikan elemen Alpine sebelum loading */
            [x-cloak] { 
                display: none !important; 
            }

            /* Tema Light Mode — Bersih dari Ungu, Dominan Putih & Biru */
            ...
            /* Override warna teks & background khusus light mode agar menjadi biru profesional */
            .light-mode .text-blue-400 { color: #2563eb !important; }
            .light-mode .hover\:text-blue-400:hover { color: #1d4ed8 !important; }
            .light-mode .bg-blue-950 { background-color: #dbeafe !important; color: #1e40af !important; }
            .light-mode .bg-blue-600 { background-color: #2563eb !important; }
            .light-mode .bg-blue-700 { background-color: #1d4ed8 !important; }
            .light-mode .border-blue-900 { border-color: #bfdbfe !important; }
        </style>
</head>
...
{{-- NAVBAR --}}
<nav class="bg-gray-900 border-b border-blue-900 px-6 py-4 sticky top-0 z-50 transition-colors duration-300">
    ...
    <div class="max-w-6xl mx-auto flex items-center justify-between">
        ...
        <a href="{{ route('user.home') }}" class="flex items-center gap-2 flex-shrink-0">
            ...
            <span class="text-blue-400 text-xl font-bold transition-colors">Embud Creative</span>
        </a>

        {{-- Desktop Menu --}}
        <div class="hidden md:flex items-center gap-5">
            <a href="{{ route('user.home') }}"
               class="text-sm hover:text-blue-400 transition {{ request()->routeIs('user.home') ? 'text-blue-400 font-semibold' : 'text-gray-300' }}">
                <i class="fas fa-home mr-1"></i> Beranda
            </a>
            <a href="{{ route('user.orders.my') }}"
               class="text-sm hover:text-blue-400 transition {{ request()->routeIs('user.orders.my') ? 'text-blue-400 font-semibold' : 'text-gray-300' }}">
                <i class="fas fa-ticket mr-1"></i> Tiket Saya
            </a>
            <a href="{{ route('user.wishlist') }}"
               class="text-sm hover:text-blue-400 transition {{ request()->routeIs('user.wishlist') ? 'text-blue-400 font-semibold' : 'text-gray-300' }}">
                <i class="fas fa-heart mr-1"></i> Wishlist
            </a>

            {{-- Notifikasi Desktop --}}
            <div class="relative" x-data="{ openNotif: false }">
                <button @click="openNotif = !openNotif" class="relative text-gray-300 hover:text-blue-400 transition flex items-center">
                ...
                <div x-show="openNotif" @click.away="openNotif = false" x-cloak
                    ...
                    <div class="max-h-72 overflow-y-auto">
                        ...
                        <div class="px-4 py-2"><p class="text-blue-400 text-xs font-bold uppercase">🆕 Konser Baru</p></div>
                        ...
                        <a href="{{ route('user.konser.show', $k) }}"
                           class="flex gap-3 items-start px-4 py-3 hover:bg-gray-700 transition border-b border-gray-700">
                            <div class="w-8 h-8 bg-blue-950 rounded-lg flex items-center justify-center flex-shrink-0">
                            ...
                            <span class="ml-auto bg-blue-900 text-blue-300 text-xs px-2 py-0.5 rounded-full">Baru</span>
                        </a>
                        @endforeach
                        @endif
                    ...
                    <div class="px-4 py-3 border-t border-gray-700">
                        <a href="{{ route('user.home') }}" class="text-blue-400 text-xs hover:underline">Lihat semua konser →</a>
                    </div>
                </div>
            </div>

            {{-- Dark/Light Mode Button --}}
            <button onclick="toggleMode()" class="text-gray-300 hover:text-blue-400 transition">
            ...
            <div class="relative" x-data="{ open: false }">
                <button @click="open = !open" class="flex items-center gap-2 text-gray-300 hover:text-white text-sm focus:outline-none">
                    ...
                    @else
                    <div class="w-8 h-8 bg-blue-700 rounded-full flex items-center justify-center font-bold text-white text-xs">
        ...
        {{-- Mobile Header Buttons (Hanya Muncul di HP) --}}
        <div class="flex md:hidden items-center gap-4">
            ...
            <div class="relative" x-data="{ openNotif: false }">
                ...
                <div x-show="openNotif" @click.away="openNotif = false" x-cloak
                    ...
                    <div class="max-h-60 overflow-y-auto">
                        ...
                        <a href="{{ route('user.konser.show', $k) }}"
                           class="flex gap-3 items-start px-4 py-3 hover:bg-gray-700 transition border-b border-gray-700">
                            <div class="w-7 h-7 bg-blue-950 rounded-lg flex items-center justify-center flex-shrink-0">
                                <i class="fas fa-music text-blue-400 text-xs"></i>
                            </div>
                            <div>
                                <p class="text-white text-xs font-semibold">{{ $k->nama_konser }}</p>
            ...
            {{-- Tombol Hamburger Menggunakan State mobileMenu --}}
    ...
    <div x-show="mobileMenu" x-transition @click.away="mobileMenu = false" x-cloak
         class="md:hidden mt-4 pt-3 border-t border-blue-900 space-y-1">
        <a href="{{ route('user.home') }}"
           class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm {{ request()->routeIs('user.home') ? 'bg-blue-950 text-blue-400 font-bold' : 'text-gray-300 hover:bg-gray-800' }}">
            <i class="fas fa-home w-5 text-center"></i> Beranda
        </a>
        <a href="{{ route('user.orders.my') }}"
           class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm {{ request()->routeIs('user.orders.my') ? 'bg-blue-950 text-blue-400 font-bold' : 'text-gray-300 hover:bg-gray-800' }}">
            <i class="fas fa-ticket w-5 text-center"></i> Tiket Saya
        </a>
        <a href="{{ route('user.wishlist') }}"
           class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm {{ request()->routeIs('user.wishlist') ? 'bg-blue-950 text-blue-400 font-bold' : 'text-gray-300 hover:bg-gray-800' }}">
            <i class="fas fa-heart w-5 text-center"></i> Wishlist
        </a>
        <a href="{{ route('user.profile.edit') }}"
           class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm {{ request()->routeIs('user.profile.edit') ? 'bg-blue-950 text-blue-400 font-bold' : 'text-gray-300 hover:bg-gray-800' }}">
        ...
        <div class="flex items-center justify-between px-3 py-3 bg-gray-950/40 rounded-lg my-2">
            ...
            <button onclick="toggleMode()" class="text-gray-300 hover:text-blue-400 transition p-1">
...
<div id="chatbot" x-data="{ open: false }" class="fixed bottom-6 right-4 z-50">
    <button @click="open = !open"
            class="w-14 h-14 bg-blue-600 hover:bg-blue-700 rounded-full flex items-center justify-center shadow-lg transition focus:outline-none">
        <i class="fas fa-robot text-white text-xl" x-show="!open"></i>
        <i class="fas fa-times text-white text-xl" x-show="open" x-cloak></i>
    </button>

    <div x-show="open" x-cloak
         class="absolute bottom-16 right-0 w-72 sm:w-80 bg-gray-900 border border-blue-700 rounded-2xl shadow-2xl overflow-hidden">
        <div class="bg-blue-700 px-4 py-3 flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-blue-200 text-xs">Tanya apapun tentang konser!</p>
            </div>
        </div>
        <div id="chat-messages" class="h-64 overflow-y-auto p-4 space-y-3">
            <div class="flex gap-2">
                <div class="w-6 h-6 bg-blue-700 rounded-full flex items-center justify-center flex-shrink-0 mt-1">
        ...
        <div class="border-t border-gray-700 p-3 flex gap-2">
            <input type="text" id="chat-input"
                   ...
                   class="flex-1 bg-gray-800 border border-gray-600 rounded-xl px-3 py-2 text-white text-xs focus:border-blue-500 focus:outline-none"
                   onkeypress="if(event.key==='Enter') sendChat()">
            <button onclick="sendChat()"
                    class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded-xl transition">
...
<script>
    // Set Default Mode ke 'light' biar langsung putih terang dari awal
    const savedMode = localStorage.getItem('mode') || 'light';
    
    // Fungsi sinkronisasi class body dan icon tema
    ...
    // Jalankan applyMode saat halaman selesai dimuat pertama kali
    applyMode(savedMode);
    ...
    function sendChat() {
        ...
        messages.innerHTML += `<div class="flex gap-2 justify-end">
            <div class="bg-blue-600 rounded-2xl rounded-tr-none px-3 py-2 max-w-xs">
